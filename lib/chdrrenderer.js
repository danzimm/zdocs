...
module.exports = function(path, options) {
	...
	var prettyParameters = function(parameters) {
		...
		if (parameters.length > 0) {
			retval += "<code>" + (parameters[0].type ? (parameters[0].type.slice(-1) == "*" ? commentParser.escapeType(parameters[0].type) : commentParser.escapeType(parameters[0].type) + " ") : "") + "</code><span class=\"parameter\">" + parameters[0].name + "</span>";
		}
		for (i = 1; i < parameters.length; i++) {
			retval += ", <code>" + (parameters[i].type ? (parameters[i].type.slice(-1) == "*" ? commentParser.escapeType(parameters[i].type) : commentParser.escapeType(parameters[i].type) + " ") : "") + "</code><span class=\"parameter\">" + parameters[i].name + "</span>";
		}
		return retval + ")";
	};
	...
	if (file) {
        var classes = [], functions = [], globals = [], protocols = [], filedescription = sortedDictionary.create(), title = null, data,
            data2, result, reger = /@interface[\s]+([\w][\w\d]{0,})[\s]{0,}:[\s]{0,}([\w][\w\d]{0,})[\s]{0,}([\s\S]*?)@end/, dict, body, props, meths;
        // blocks
        /// {{{
        var clsCodeBlocks = {
            "@property\\s{0,}\\(([\\w\\s,]*)\\)\\s{0,}(\\w[\\w\\d]{0,})((?:[\\s*]|<(?:\\w[\\w\\d]{0,},?){0,}>)+)(\\w[\\w\\d]{0,});" : function(result, update) {
                var attr, t,
                ...
                t = result.splice(0,2);
                dict.addObject(t[0] + (t[1].indexOf("*") != -1 ? t[1] : t[1].trim()), "type");
                dict.addObject(result.splice(0,1)[0], "name");
                dict.addObject("property", "zdocs");
                update(result);
                return dict;
            },
            "([-+])\\s{0,}\\(\\s{0,}(\\w[\\w\\d]{0,}\\s{0,}\\*{0,})\\s{0,}\\)\\s{0,}(\\w[\\w\\d]{0,}\\s{0,}(?::\\(\\w[\\w\\d]{0,}(?:[\\s*]|<(?:\\w[\\w\\d]{0,},?){0,}>){0,}\\)\\w[\\w\\d]{0,})?(?:\\s{0,}(?:\\w[\\w\\d]{0,})?:\\(\\w[\\w\\d]{0,}(?:[\\s*]|<(?:\\w[\\w\\d]{0,},?){0,}>){0,}\\)\\w[\\w\\d]{0,}){0,})\\s{0,};" : function(result, update) {
        ...
        var codeBlocks = {
            ...
            "@protocol\\s{0,}(\\w[\\w\\d]{0,})[^;]*?\\n([\\s\\S]*?)@end" : function(result, update) {
                var dict = sortedDictionary.create(),
                    body, data, props = [], meths = [];
                dict.addObject(result.splice(0,1)[0],"name");
                body = result.splice(0,1)[0];
                ...
                dict.addObject(props, "properties");
                dict.addObject(meths, "methods");
                dict.addObject("protocol", "zdocs");
                update(result);
                return dict;
            },
            "(\\w[\\w\\d\\s]*?)((?:[\\s*]|<(?:\\w[\\w\\d]{0,},?){0,}>)+)(\\w[\\w\\d]*)\\(([^)]*?)\\)\s{0,}(?:;|\\{)" : function(result, update) {
                var paren = result.splice(-1)[0],
                    doer = /(\w[\w\d]*)((?:[\\s*]|<(?:\\w[\\w\\d]{0,},?){0,}>)+)(\w[\w\d]*)/,
                    dict = sortedDictionary.create(),
                    t;
                ...
                t = result.splice(0,2);
                dict.addObject({
                    type : t[0] + (t[1].indexOf("*") != -1 ? t[1] : t[1].trim())
                }, "retval");
                dict.addObject(result.splice(0,1)[0], "name");
        ...
        while (true) {
            result = reger.exec(data.text);
            if (result == null)
                break;
            data.text = data.text.replace(result.splice(0,1)[0],""); //get rid of first thing
            props = [];
            meths = [];
            dict = sortedDictionary.create();
            dict.addObject(result.splice(0,1)[0],"cls");
            dict.addObject(result.splice(0,1)[0],"supercls");
            body = result.splice(0,1)[0];
            data2 = commentParser.extract(body, commentBlocks, clsCodeBlocks);
            data2.couples.forEach(function(elm) {
                if (elm['zdocs'] == 'property') {
                    props.push(elm);
                } else if (elm['zdocs'] == 'method') {
                    meths.push(elm);
        ...
        reger = /@protocol\s{0,}(\w[\w\d]{0,})[^;]*?\n([\s\S]*?)@end/;
        while (true) {
            result = reger.exec(data.text);
            if (result == null)
                break;
            data.text = data.text.replace(result.splice(0,1)[0],"");
            props = [];
            meths = [];
            dict = sortedDictionary.create();
            dict.addObject(result.splice(0,1)[0],"name");
        ...
        data.couples.forEach(function(couple) {
            if (couple.hasKey('zdocs')) {
                if (couple['zdocs'] === 'class') {
                    ...
                    globals.push(couple);
                } else if (couple['zdocs'] === 'protocol') {
                    protocols.push(couple);
                }
            }
        });
        ...
        classes.forEach(function(cls, ind) {
            cls.methods.forEach(function(meth, i) {
                ...
                if (!meth.items) {
                    ...
                    if (meth.parameters && meth.parameters.length > 0) {
                        ...
                        var descs = meth.parameters.map(function(parameter) {
                            return "Type: <code>" + commentParser.escapeType(parameter.type) + "</code>" +  (parameter.description ? "<br/>" + parameter.description : "");
                        });
                    ...
                    if (meth.retval) {
                        meth.items.addObject("Type: <code>" + commentParser.escapeType(meth.retval.type) + "</code>" + (meth.retval.description ? "<br/>" + meth.retval.description : ""), "Return Value");
                    }
                    if (meth.discussion) {
                ...
                if (!meth.fullname) {
                    meth.addObject((meth.clsmeth == 'true' ? '+' : '-') + " (<code>" + (meth.retval ? commentParser.escapeType(meth.retval.type) : "Unknown") + "</code>)" + prettySelector(meth), "fullname");
                }
            ...
            cls.properties.forEach(function(prop, i) {
                ...
                if (!prop.items) {
                    ...
                    if (prop.type) {
                        prop.items.addObject("<code>" + commentParser.escapeType(prop.type) + "</code>", "Type");
                    }
                    if (prop.attributes) {
                ...
                if (!prop.fullname) {
                    var proptype, lc;
                    if (prop.type) {
                        lc = prop.type[prop.type.length-1];
                        if (/[\w\d]/.test(lc)) {
                            proptype = prop.type + " ";
                        } else {
                            proptype = prop.type;
                    ...
                    proptype = commentParser.escapeType(proptype);
                    prop.addObject("@property " + (prop.attributes ? "(" + prop.attributes.join(", ") + ") " : "") + "<code>" + proptype + "</code><span class=\"parameter\">" + (prop.name ? prop.name : "VARNAME") + "</span>", "fullname");
                }
                page.addDocumentationBlock(prop);
            });
        });
        protocols.forEach(function(proto, index, arr) {
            proto.methods.forEach(function(meth, i) {
                meth.addObject(proto.name, "category");
                if (!meth.group) {
                    meth.addObject("Methods", "group");
                }
                if (!meth.items) {
            ...
            proto.properties.forEach(function(prop, i) {
                prop.addObject(proto.name, "category");
                if (!prop.group) {
                    prop.addObject("Properties", "group");
                }
                if (!prop.items) {
        ...
        functions.forEach(function(func,index, arr) {
            ...
            if (!func.items) {
                ...
                if (func.parameters && func.parameters.length > 0) {
                    ...
                    var descs = func.parameters.map(function(parameter) {
                        return "Type: <code>" + commentParser.escapeType(parameter.type) + "</code><br/>" +  parameter.description
                    });
                ...
                if (func.retval) {
                    func.items.addObject("Type: <code>" + commentParser.escapeType(func.retval.type) + "</code>" + (func.retval.description ? "<br/>" + func.retval.description : ""), "Return Value");
                }
                if (func.discussion) {
            ...
            if (!func.fullname) {
                // should i do what i do with proptype up above?
                func.addObject("<code>" + (func.retval ? commentParser.escapeType(func.retval.type) + " " : "") + "</code>" + func.name + prettyParameters(func.parameters), "fullname");
            }
        ...
        globals.forEach(function(glob,index, arr) {
            ...
            if (!glob.items) {
                ...
                if (glob.type) {
                    glob.items.addObject("<code>" + commentParser.escapeType(glob.type) + "</code>", "Type");
                }
                if (glob.discussion) {
            ...
            if (!glob.fullname) {
                glob.addObject("<code>" + (glob.type ? commentParser.escapeType(glob.type) + " " : "") + "</code>" + glob.name, "fullname");
            }
            page.addDocumentationBlock(glob);
        });
        page.appendScript({
            inner : "hljs.configure({ langauges : 'c', 'objective-c' });"
        });
        return page.render();
	}
	return "";
