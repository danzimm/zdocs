...
module.exports = function(path, options) {
	...
	if (file) {
        var classes = [], functions = [], globals = [], filedescription = sortedDictionary.create(), title = null, data, data2,
            result, clsreger = /@interface[\s]+([\w][\w\d]{0,})[\s]{0,}:[\s]{0,}([\w][\w\d]{0,})[\s]{0,}([\s\S]*?)@end/g, dict, body, props, meths;
        // blocks
        /// {{{
        var clsCodeBlocks = {
        ...
        var codeBlocks = {
            "@interface[\\s]+([\\w][\\w\\d]{0,})[\\s]{0,}:[\\s]{0,}([\\w][\\w\\d]{0,})[\\s]{0,}([\\s\\S]*?)@end" : function(result, update) {
                ...
                logger("Got cls data: " + JSON.stringify(data));
        ...
        while (true) {
            result = clsreger.exec(data.text);
            if (result == null)
                break;
            result.splice(0,1); //get rid of first thing
            props = [];
            meths = [];
            dict = sortedDictionary.create();
            dict.addObject(result.splice(0,1)[0],"cls");
