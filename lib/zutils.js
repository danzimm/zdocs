                    ...
                    if (into.isSorted) {
                        if (into.hasKey(k)) {
                            if (typeof into[k] == 'object' && typeof from[k] == 'object' && !Array.isArray(into[k]) && !Array.isArray(from[k])) {
                                ...
                                into[k] = mergecb(path+'/'+key,into[k],from[k]);
                            }
                        } else {
...
var commentParser = (function() {
    ...
    that.extract = function(text, commentBlocks, codeBlocks, merger) {
        var i,j,
            reger, result,
            comment, tmp, found,
            retval = [], match, comms = [],
            comDict, codDict, dict;
        if (!merger) {
